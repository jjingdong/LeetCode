...
class Solution:
    ...
    def spiralMatrixIII(self, rows: int, cols: int, rStart: int, cStart: int) -> List[List[int]]:
        ...
        step_size = 2 * max(size, col_size)

        for s in range(1, step_size, 2):
            # print(f's = {s}')
            # right, down, left, up
            dirs = [(0, 1, s), (1, 0, s), (0, -1, s + 1), (-1, 0, s + 1)]
            for dr, dc, ds in dirs:
                ...
                for _ in range(ds):
                    ...
                    if 0 <= r < size and 0 <= c < col_size:
                        result.append([r, c])
                        if len(result) == size * col_size:
                            return result
