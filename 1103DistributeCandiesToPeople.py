...
class Solution:
    ...
    #        candies = unlimited, num_people = 4 = n
    #         1   2   3   4     -> total = (n+1)*n/2            -> total = (n+1)*n/2 + n*n*(row-1)
    #         5   6   7   8     -> total = (n+1)*n/2 + n*n      ->
    #         9   10  11  12    -> total = (n+1)*n/2 + n*n*2    ->
    #         13  14  15  16
    #        assume row = 3:
    #        (n+1)*n/2 + n*n*(row-3) + (n+1)*n/2 + n*n*(row-2) + (n+1)*n/2 + n*n*(row-1) = candies

    # Time O(KN), K = no. of rows. Space O(1) runtime = 36 ms
